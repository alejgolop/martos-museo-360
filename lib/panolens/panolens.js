...
addEventListener( 'deviceorientation', (event)=>{
	orientationParams.a=event.alpha | 0;
	orientationParams.b=event.beta | 0;
	orientationParams.g=event.gamma | 0;
	console.log(event)
	$(".testing").text(`A:${event.alpha} B:${event.beta} G:${event.gamma} c:${JSON.stringify(viewer.camera.rotation)}`);
  }
	);
...
(function (global, factory) {
	...
	function Spot(scale = 10,imageSrc,animated)
	{
	...
	Spot.prototype = Object.assign( Object.create( THREE.Mesh.prototype ), {
	    constructor: Spot,
		/**
	    ...
	    translateElement: function ( x, y ) {
	        ...
	        if ( ( this.mode === MODES.CARDBOARD || this.mode === MODES.STEREO ) 
				...
				console.log(`translate(${left}px, ${top}px) rotateZ(${-orientationParams.a}deg)`);
				var newTransform=`translate(${left}px, ${top}px)`;

				if(viewer.control instanceof DeviceOrientationControls)
				{
					this.setElementStyle( 'transform', element, `${newTransform} rotateZ(${orientationParams.a-45}deg)` );

				}else{
					this.setElementStyle( 'transform', element, newTransform );
	     ...
	     * @instance
	     */
	    addHoverText: function ( text, delta = 60 ) {
	     ...
	     * @instance
	     */
	    addHoverElement: function ( el, delta = 60 ) {
	    ...
	    focus: function ( duration, easing ) {
	        ...
	        if ( this.HANDLER_FOCUS ) {
				...
				var copiedPosition=this.position.clone();
				copiedPosition.x*=-1;

	            this.HANDLER_FOCUS( copiedPosition, duration, easing );
	            this.onDismiss();
	...
	Panorama.prototype = Object.assign( Object.create( THREE.Mesh.prototype ), {
	    ...
	    toggleInfospotVisibility: function ( isVisible, delay ) {
	        ...
	        this.traverse( function ( object ) {
	            ...
	            if ( object instanceof Infospot || object instanceof Spot) {
	    ...
	    link: function ( pano, position, imageScale, imageSrc ) {
			...
			const spot=new PANOLENS.Spot(scale,img);

			var vector=position.multiplyScalar(0.9);
			vector.x*=-1;
	        ...
	        this.add( spot );
			spot.lookAt(viewer.camera.position);
	...
	Viewer.prototype = Object.assign( Object.create( THREE.EventDispatcher.prototype ), {
	    ...
	    update: function () {
	        ...
	        this.scene.traverse( function( child ){
	            if ( (child instanceof Infospot || child instanceof Spot)
					&& child.element 
					&& ( this.hoverObject === child 
						|| child.element.style.display !== 'none' 
						|| (child.element.left && child.element.left.style.display !== 'none')
						|| (child.element.right && child.element.right.style.display !== 'none') ) ) {
	                if ( this.checkSpriteInViewport( child ) ||child instanceof Spot) {
